...
def dump_thermals_to_wpt_file(flight, wptfilename, endpoints=False):
    ...
    with open(wptfilename, 'wt') as wpt:
        ...
        for x, thermal in enumerate(flight.thermals):
            lat = _degrees_float_to_degrees_minutes_seconds(flight.thermals[x].enter_fix.lat, 'lat')
            lon = _degrees_float_to_degrees_minutes_seconds(flight.thermals[x].enter_fix.lon, 'lon')
            wpt.write("%02d        " % x)
            ...
            if endpoints:
                lat = _degrees_float_to_degrees_minutes_seconds(flight.thermals[x].exit_fix.lat, 'lat')
                lon = _degrees_float_to_degrees_minutes_seconds(flight.thermals[x].exit_fix.lon, 'lon')
                wpt.write("%02dEND     " % x)
                wpt.write("%s %02d %02d %05.2f    " % (lat.hemisphere, lat.degrees, lat.minutes, lat.seconds))
                wpt.write("%s %03d %02d %05.2f     " % (lon.hemisphere, lon.degrees, lon.minutes, lon.seconds))
